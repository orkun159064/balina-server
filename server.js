...
app.post('/api/check-trial', (req, res) => {
    ...
    if (user && user.sub && user.subEnd && user.subEnd > Date.now()) {
        ...
        return res.json({ allowed: true, start: trial ? trial.start : Date.now(), subscribed: true, isAdmin: false, subEnd: user.subEnd });
    }
    const trial = trials[email];
    if (trial) {
        if ((Date.now() - trial.start) >= TRIAL_MS) {
            return res.json({ allowed: true, start: trial.start, subscribed: false, isAdmin: false, expired: true });
        }
